...
function getPosts() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const result = data.posts.map((post) => {
            ...
            const id = post.id;
            // get all the tags
            const generalTags = post.tags.general.map((tag_name) => { return { name: tag_name, type: MetaData_1.TagType.General, score: getInitialTagScore(tag_name, MetaData_1.TagType.General) }; });
            const artistTags = post.tags.artist.map((tag_name) => { return { name: tag_name, type: MetaData_1.TagType.Artist, score: ARTIST_TAG_SCORE }; });
            const speciesTags = post.tags.species.map((tag_name) => { return { name: tag_name, type: MetaData_1.TagType.Species, score: getInitialTagScore(tag_name, MetaData_1.TagType.Species) }; });
            const characterTags = post.tags.character.map((tag_name) => { return { name: tag_name, type: MetaData_1.TagType.Character, score: CHARACTER_TAG_SCORE }; });
...
function getInitialTagScore(tag_name, tag_type) {
    if (tag_type === MetaData_1.TagType.General) {
...
//# sourceMappingURL=fetching_utility_backend.js.map